...
export default {
  ...
  data() {
    return {
      title: "ข้อมูลบุคลากรในสำนักงานเขตพื้นที่การศึกษา",
      formTitle: "",
  ...
  methods: {
    ...
    showItem(item) {
      this.formTitle = `ข้อมูล: ${item.areaName} : (${item.areaId})`;
    ...
    gotoSchool(item, index, button) {
      let url = `/admin/schools/${item.areaId}`;
      this.$router.push(url);
    },
    gotoPersonel(item, index, button) {
      let url = `/admin/personels/${item.areaId}`;
      this.$router.push(url);
    }
  },
