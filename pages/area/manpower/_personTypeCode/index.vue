...
export default {
  ...
  data() {
    return {
      title: `อัตรากำลังในสังกัด ${this.$store.state.account.user.areaName} (${this.$store.state.account.user.areaCode})`,
      formTitle: "",
      teachers: [],
  ...
  methods: {
    async getTeacherByArea() {
      console.log(`MANPOWER AREA: ${this.$store.state.account.user.areaName} : ${this.personTypeCode}`);
      try {
    ...
    deleteItemConfirm() {
      ...
      console.log("Delete : " + this.selectedItem._id);
    ...
    editTeacher(teacher) {
      this.$router.replace(`/area/manpower/${this.personTypeCode}/${teacher._id}`);
    },
