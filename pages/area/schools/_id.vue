      ...
      <v-flex sm12 lg12>
        ...
        <v-card class="mb-4">
          <v-toolbar color="primary darken-1" dark flat dense cad>
            ...
            <v-spacer></v-spacer>
            <v-btn text icon @click="addTeacher()">เพิ่ม</v-btn>
  ...
  methods: {
    async getSchools() {
      try {
        ...
        //this.title = "รายชื่อบุคลากรในโรงเรียน" + this.school.schName;
      } catch (error) {
        console.log(error);
      }
    },
    ...
    showItem(item) {
      this.formTitle = `รายชื่อบุคลากรในสังกัด<small>ที่อยู่ในการดูแลของ โรงเรียน${item.schName} : (${item.schId})</small>`;
      ...
      this.form.fullname = `${item.prefixName.title} ${item.firstName}  ${item.lastName}`;
    ...
    addTeacher() {
      this.$router.push("/area/teachers/add?schId=" + this.schId);
    },

    //ลิ้งค์ไปหน้าแก้ไขข้อมูล
    editTeacher(teacher) {
      this.$router.push("/area/teachers/" + teacher.personId);
    },
